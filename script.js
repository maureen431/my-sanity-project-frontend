// Wait for DOM and Sanity library to load
document.addEventListener('DOMContentLoaded', function() {
  const DOCUMENT_ID = 'text-saver-entry';

  // Initialize Sanity client
  let sanityClient;
  if (window.sanity && window.sanity.createClient) {
    sanityClient = window.sanity.createClient({
      projectId: 'iv378ryl',
      dataset: 'production',
      useCdn: false,
  ...
  window.saveText = async function() {
    if (!sanityClient) {
      alert("Sanity client not initialized. Please refresh the page.");
      return;
    }
